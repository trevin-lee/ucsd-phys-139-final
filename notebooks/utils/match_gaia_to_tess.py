...
def cross_match_gaia(gaia_folder, tess_fits_dir, threshold_degree, output_filename, output_dir):
    """
    Performs cross-match between Gaia DR3 source files and TESS FITS files.

    Args:
        gaia_folder (str): Directory containing Gaia CSV or CSV.GZ files.
        tess_fits_dir (str): Directory containing TESS FITS files.
        threshold_degree (float): Match radius in degrees.
        output_filename (str): Filename of the result csv file.
    ...
    # Gaia columns (usually lowercase in DR3 exports)
    # Check if they exist, handle common variations if needed
    gaia_ra_col = 'ra'
    gaia_dec_col = 'dec'
    ...
    # 2. Load TESS Data (from FITS headers)
        ...
        for file_name in files:
            if file_name.endswith('.fits'):
                ...
                try:
                    ...
                    # Silent or minimal warning for bad files to keep output clean
                    pass
    ...
    # Create TESS DataFrame for easier matching logic integration
    ...
    # 3. Perform Cross-match (Astropy)
    print(f"⚙️  Matching with threshold: {threshold_degree} deg...")
    ...
    if 'best_class_name' in matches_gaia.columns:
        ...
        result_df['Gaia_Class'] = matches_gaia['ML_classification']
         ...
         # Just an example of pulling other useful columns if needed, 
         # but we'll stick to what was in the notebook unless requested.
         pass
